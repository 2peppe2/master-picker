...
export const FilterPopOver = () => {
    ...
    const masterProfiles = Object.keys(MastersRequirements)
    return (
        <div className="grid gap-4">
            ...
            <div className="grid gap-4">
                <div className="flex items-center gap-3">
                    <Checkbox id="showOnly" checked={filter.showOnlyApplicable} onCheckedChange={onShowOnlyApplicableChange} />
                    <Label htmlFor="showOnly" className="text-sm">Show only applicable courses</Label>
                </div>
                <Select>
                    ...
                    <SelectContent>
                        <SelectGroup>
                            ...
                            <SelectItem value="cs"><MastersBadge program={"AI"} />Computer Science</SelectItem>
                ...
                <div className="flex items-center gap-3">
                    ...
                    {range(0, filter.semester.length).map((index) => (
                        <Fragment key={index}>
                            <Checkbox id={`semester${index}`} checked={filter.semester[index]} onCheckedChange={(checked: boolean) => onSemesterChange(index, checked)}/>
                            <Label htmlFor={`semester${index}`} className="text-sm">{index + 7}</Label>
                        </Fragment>
                    ))
                    }
                </div>
                <div className="flex items-center gap-3">
                    ...
                    {range(0, filter.period.length).map((index) => (
                        <Fragment key={index}>
                            <Checkbox id={`period${index}`} checked={filter.period[index]} onCheckedChange={(checked: boolean) => onPeriodChange(index, checked)}/>
                            <Label htmlFor={`period${index}`} className="text-sm">{index + 1}</Label>
                        </Fragment>
                    ))
                    }
                </div>
                <div className="flex items-center gap-3">
                    ...
                    {range(0, filter.block.length).map((index) => (
                        <Fragment key={index}>
                            <Checkbox id={`block${index}`} checked={filter.block[index]} onCheckedChange={(checked: boolean) => onBlockChange(index, checked)}/>
                            <Label htmlFor={`block${index}`} className="text-sm">{index + 1}</Label>
                        </Fragment>
                    ))
                    }
                </div>
